                    ...
                    if (resident.hasTown() && RuinBase.isRuined(resident.getTown())) {
                        Confirmation.runOnAccept(() -> {
                            ...
                            try {
                                ...
                                if (!event.isCancelled()) FeudalismMain.plugin.getServer().getPluginManager().callEvent(event);
                            } catch (NotRegisteredException e) {
                                e.printStackTrace();
                            }
                        }).setTitle("§bAre you sure you want to reclaim " + resident.getTown() + "?");
                    } else {sender.sendMessage(ChatInfo.color("&cYou don't have a town, or your town isn't ruined."));}

                // for nation reclaiming
                case "assign":
                    if (args.length >= 3) {
                        if (sender.hasPermission(FeudalismMain.config.getString("nation-reclaiming-permission"))) {
                                ...
                                if (Bukkit.getPlayer(args[1]).isOnline() || Bukkit.getPlayer(args[1]) != null) {
                                    if (TownyUniverse.getInstance().getDataSource().getTown(args[2]) != null) {
                                        ...
                                        if (resident.getTown().getNation() == town.getNation()) {
                                            ...
                                            } else {sender.sendMessage(ChatInfo.color("&c" + args[2] + " is not ruined."));}
                                        } else {sender.sendMessage(ChatInfo.color("&c" + args[2] + " is not in your nation."));}
                                    } else {sender.sendMessage(ChatInfo.color("&c" + args[2] + " is not registered."));}
                                } else {sender.sendMessage(ChatInfo.color("&c" + args[1] + " is not online."));}
                            } else {sender.sendMessage(ChatInfo.color("&cYou don't have a nation or a town."));}
                        } else {sender.sendMessage(ChatInfo.color("&cYou don't have enough permissions."));}
